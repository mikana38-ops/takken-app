...
async function removeWrongId(id){const cur=await loadWrongIds();await saveWrongIds(cur.filter(x=>x!==id));}
...
// 起動時にストレージから間違い問題を読み込む
useEffect(()=>{loadWrongIds().then(ids=>setWrongIds(ids));},[]);
...
const start=(isReview=false)=>{
  let pool;
  if(isReview){
    pool=Q.filter(q=>wrongIds.includes(q.ref||q.q.slice(0,20)));
...
const answer=useCallback(async(v)=>{
  ...
  const c=cards[idx];
  const key=c.ref||c.q.slice(0,20);
  const ok=v===c.a;
  if(ok){
    setScore(s=>s+1);
    // 正解したら間違いリストから削除
    const newIds=wrongIds.filter(x=>x!==key);
    setWrongIds(newIds);
    await removeWrongId(key);
  }else{
    // 不正解なら間違いリストに追加
...
{/* 復習ボタン */}
{wrongIds.length>0&&(
  <button onClick={()=>start(true)} style={{width:"100%",padding:"11px 14px",borderRadius:12,border:"2px solid #e74c3c",background:"rgba(231,76,60,.1)",color:"#e74c3c",fontSize:14,fontWeight:700,cursor:"pointer",marginBottom:10,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
    <span>🔴 間違えた問題を復習する</span>
    <span style={{background:"#e74c3c",color:"#fff",borderRadius:20,padding:"2px 10px",fontSize:12}}>{wrongIds.length}問</span>
  </button>
)}
...
<button onClick={()=>setCat("all")} style={{width:"100%",padding:"9px 12px",borderRadius:10,marginBottom:10,display:"flex",justifyContent:"space-between",alignItems:"center",border:cat==="all"?"2px solid var(--accent)":"1px solid var(--border)",background:cat==="all"?"rgba(108,99,255,.14)":"var(--card)",color:cat==="all"?"var(--accent)":"var(--text)",fontSize:13,fontWeight:700,cursor:"pointer"}}>
  <span>📚 全セクション</span><span style={{fontSize:11,color:"var(--text2)"}}>{Q.length}問</span>
</button>
...
{GROUPS.map(g=>{
  ...
  return(
    <div key={g.label} style={{marginBottom:8}}>
      <div style={{fontSize:10,fontWeight:700,color:"var(--text2)",marginBottom:4,paddingLeft:2}}>{g.label} <span style={{color:"var(--border)"}}>({gTotal}問)</span></div>
      <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
        {gCats.map(id=>{
          const ci=CATS.find(c=>c.id===id);
          const n=Q.filter(q=>q.cat===id).length;
          const sel=cat===id;
          const wCount=wrongIds.filter(wid=>Q.find(q=>(q.ref||q.q.slice(0,20))===wid&&q.cat===id)).length;
...
<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
  ...
  <div style={{display:"flex",gap:8,alignItems:"center"}}>
    {reviewMode&&<span style={{fontSize:10,background:"rgba(231,76,60,.2)",color:"#e74c3c",borderRadius:6,padding:"2px 6px",fontWeight:700}}>復習</span>}
...
<div style={{textAlign:"center",marginBottom:20}}><div style={{fontSize:44,marginBottom:6}}>{em}</div><h2 style={{fontSize:19,fontWeight:800,margin:"0 0 4px"}}>{ms}</h2>{reviewMode&&<div style={{fontSize:12,color:"var(--red)",fontWeight:700}}>復習モード</div>}</div>
...
<div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
  ...
  {wrongIds.length>0&&<button onClick={()=>start(true)} style={{flex:1,padding:12,borderRadius:11,border:"2px solid var(--red)",background:"rgba(231,76,60,.1)",color:"var(--red)",fontSize:13,fontWeight:700,cursor:"pointer"}}>🔴 復習 ({wrongIds.length}問)</button>}
